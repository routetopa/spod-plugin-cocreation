...
// # Initialization
  ...
  initializeRows: function(rows){
    ...
    if(_.isArray(rows)){
      //ISISLab code
      //visible rows for pagination
      this.allRows = rows;
      //to improve just for test
      this.rows = rows.slice(0,config.DEFAULT_CHUNK_SIZE);
      //END ISISLab code
    ...
    this.rows = this.rows.slice(0,config.DEFAULT_CHUNK_SIZE);
    //END ISISLab code
  },
...
// # Rows
  ...
  rowExists: function(row_id){
    ...
    //return _.include(this.allRows,row_id);
  },
  ...
  indexForRow: function(row_id){
    ...
    return _.indexOf(this.allRows,row_id);
  },
  ...
  insertRow: function(position, id){
    ...
    this.allRows.splice(position,0,new_id);
  ...
  deleteRow: function(row_id){
    ...
    var row_pos = _.indexOf(this.allRows,row_id);
    if(row_pos === -1) return false;
    this.rows.splice(row_pos,1);
    this.allRows.splice(row_pos,1);
